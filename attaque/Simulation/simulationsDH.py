...
def derive_key(shared_secret):
    ...
    print(f"Clé AES dérivée (base64) : {base64.b64encode(key).decode()}")
    return key
...
def aes_gcm_encrypt(key, plaintext, associated_data=b""):
    ...
    tag = encryptor.tag
    print(f"Nonce généré (base64) : {base64.b64encode(nonce).decode()}")
    print(f"Texte chiffré (base64) : {base64.b64encode(ciphertext).decode()}")
    print(f"Tag d'authentification (base64) : {base64.b64encode(tag).decode()}")
...
def rsa_sign(message, private_key):
    ...
    print(f"Signature RSA générée (base64) : {base64.b64encode(signature).decode()}")
    return signature
...
def rsa_verify(message, signature, public_key):
    try:
        ...
        print("Vérification de la signature RSA : Réussie")
        return True
    except Exception as e:
...
def server():
    ...
    print(f"Clé publique RSA du serveur (base64) : {base64.b64encode(serialize_public_key(server_rsa_public)).decode()}")
    ...
    print(f"Clé publique DH du serveur (base64) : {base64.b64encode(serialize_public_key(server_dh_public)).decode()}")

    try:
        conn, addr = server_socket.accept()
        conn.settimeout(None)  # Pas de timeout pour les tests
        with conn:
            ...
            print(f"Secret partagé calculé (base64) : {base64.b64encode(shared_secret).decode()}")
            ...
            while True:
                ...
                print("Message chiffré reçu du client :")
                ...
                if message and rsa_verify(message, signature, client_rsa_public):
                    ...
                    # Répondre au client
                    response = input("Serveur : Entrez votre réponse : ")
                    if not response:
                    ...
                    print("Réponse chiffrée envoyée au client :")
...
def client():
    ...
    client_socket.settimeout(None)  # Pas de timeout pour les tests
    ...
    print(f"Clé publique RSA du client (base64) : {base64.b64encode(serialize_public_key(client_rsa_public)).decode()}")
    try:
        ...
        print(f"Clé publique DH du client (base64) : {base64.b64encode(serialize_public_key(client_dh_public)).decode()}")
        ...
        while True:
            ...
            message = input("Client : Entrez votre message (ou 'quit' pour terminer) : ")
            ...
            print("Réponse chiffrée reçue du serveur :")
            print(json.dumps(response_data, indent=2))
