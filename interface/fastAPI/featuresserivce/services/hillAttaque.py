...
def load_hill_matrices(block_size=3):
    ...
    if block_size == 3:
        input_file = Path(__file__).parent /'hill_matrices.json'
    elif block_size == 2:
        input_file = Path(__file__).parent /'hill_matrices_2x2.json'
    else:
...
def is_valid_ciphertext(ciphertext: str, block_size: int):
    """
    Vérifie si le texte chiffré est valide pour le Hill Cipher.

    Arguments :
    - ciphertext : str : Le texte chiffré à valider.
    - block_size : int : La taille des blocs (2 ou 3 en général).

    Retourne :
    - bool : True si le texte est valide, False sinon.
    """
    # Vérifier que le texte contient uniquement des lettres majuscules (A-Z)
    if not re.fullmatch(r'[A-Z]+', ciphertext):
        return False
    
    # Vérifier que la longueur du texte est un multiple de la taille du bloc
    if len(ciphertext) % block_size != 0:
        return False
